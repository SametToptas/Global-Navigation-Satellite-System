function [Trzd,Trzw,ME] =trop_SPP(lat,D,H,E)

%The tropospheric model developed by [Collins, 1999] and adopted by 
%the Satellite-Based Augmentation System (SBAS). Details of the model are
%presented in: GNSS Data Processing. J. Sanz Subirana, J.M. Juan Zornoza 
%and M. Hernández-Pajares, 2013, Vol. 1, (ESA TM-23/1, May 2013),
%pp.123-124
 
%Inputs
% lat: Latitude of the receiver [deg]
% D: Day of year [integer]
% H: Height of the station [m]
...
%Yazgý Nur Sayin, Hacettepe University. 05.03.2021


%Average meteorological parameters for the tropospheric delay:
%pressure [P(mbar)], temperature [T(K)], water vapour pressure [e(mbar)], 
%temperature rate [Beta(K/m)] and water vapour rate [Alpha (Dimensionless)]
...
ME = 1.001/(sqrt(0.002001+(sin(E)^2)));
...
P =     A(1)+B(1)*cos((2*pi*(D-Dmin))/365.25);
T =     A(2)+B(2)*cos((2*pi*(D-Dmin))/365.25);
e =     A(3)+B(3)*cos((2*pi*(D-Dmin))/365.25);
Beta =  A(4)+B(4)*cos((2*pi*(D-Dmin))/365.25);
Alpha = A(5)+B(5)*cos((2*pi*(D-Dmin))/365.25);
